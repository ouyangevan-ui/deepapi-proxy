...
| one-api package limits | Product + finance owners | Follow `ONEAPI-LIMITS-RUNBOOK.md` and use `ops/verify-live-limits.example.sh` or equivalent no-secret commands | Redacted one-api user/token/group settings, status summaries, usage logs, balance/quota deltas, and provider usage check | Test users are 60/min, 1000/hour, concurrency 3; Starter is 120/min, 3000/hour, concurrency 5; Vision is 10/min, 100/hour, concurrency 1-2; text and vision quota are separate |
