...
| Rate and concurrency enforcement | Product + finance owners | With a test key, exercise an ordinary model and deepapi-vision against one-api user/token/group settings | Redacted request windows, one-api settings screenshots, usage logs, balance deltas, and provider usage check | Configured rate limit, concurrency limit, balance deduction, and over-limit behavior match policy; rejected requests create no upstream usage |
