...
The initial product exposes only public DeepAPI model names:
`deepapi-everyday`, `deepapi-advanced`, and `deepapi-vision`, through an
OpenAI-compatible protocol. It does not provide OpenAI models or imply an
...
Upstream names, including `deepseek-*`, `qwen-*`, `gpt-*`, `claude-*`, and
`gemini-*`, are administrator-only mapping and provider records. Ordinary
requests using those names fail closed with no upstream usage.
